...
public partial class AuthViewModel(IAuthApi authApi, AuthService authService) : BaseViewModel
{
    ...
    public bool CanSignup => CanSignin
        ...
        [ObservableProperty]
        private SignupRequestDto _pendingSignup;

        [RelayCommand]
        private async Task SignupAsync()
        {
        _pendingSignup = new SignupRequestDto(FirstName, LastName, Gender, Email, Password, Birthdate);
         await GoToAsync($"//{nameof(RegisterAgreement)}", animate: true);
    }
    public async Task<bool> TrySignupAsync(SignupRequestDto signupRequest)
    {
        var result = await _authApi.SignupAsync(signupRequest);
        if (result.IsSuccess)
        {
        ...
        {
            // Hata mesajı döndürmek için başka bir mekanizma kullanabilirsiniz.
            return false;
        }
    }
    //[RelayCommand]
    ...
    //    try
    //    {
    //        var result = await _authApi.SignupAsync(_pendingSignup);

    //        if (result.IsSuccess)
    //        {
    //            _authService.Signin(result.Data);

    //            await GoToAsync($"//{nameof(LoginPage)}", animate: true);
    //        }
    //        else
    //        {
